...
def save_jp_report(
    ...
    if no_edinet:
        # No yuho yet: render the section skeleton so the reader sees what
        # WILL be covered once the yuho lands, with each narrative section
        # showing a "待 FYxxxx Q4 有報發布後更新" placeholder. Section 1
        # (財務速報) and Section 8 (其他重大事件) carry the real content
        # we have from TDNET; narratives 2–7 are deliberate placeholders.
        placeholder = f"> 待 FY{fy_label} 有報發布後更新。\n"

        lines.append("## 1. 財務速報")
        lines.append("")
        lines.append(_render_tdnet_financial_highlights(recent_events or []))

        lines.append("## 2. 公司概況（事業の内容）")
        lines.append("")
        ...
        if other_events:
            lines.append("## 8. 其他重大事件（過去 12 個月）")
            lines.append("")
